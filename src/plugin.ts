...
export class Spotify extends Plugin {
	...
	public makeRequest<T>(
		endpoint: string,
		modify: ModifyRequest = () => void 0,
	): Promise<T> {
		const req = fetch(
			`${BASE_URL}${/^\//.test(endpoint) ? endpoint : `/${endpoint}`}`,
	...
	private async getAlbumTracks(id: string): Promise<Result> {
		...
		const tracks = album.tracks.items
			.filter((e) => this.filterNullOrUndefined(e))
			.map((item) => Spotify.convertToUnresolved(item));
	...
	private async getPlaylistTracks(id: string): Promise<Result> {
		const playlist = await this.makeRequest<Playlist>(
	...
	private async getTrack(id: string): Promise<Result> {
		const data = await this.makeRequest<SpotifyTrack>(
			`${BASE_URL}/tracks/${id}`,
		);
